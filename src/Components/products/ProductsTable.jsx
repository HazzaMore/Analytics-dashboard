...
const ProductsTable = () => {
	...
	const handleSearch = (e) => {
		// Removes case sensitivity during search
	...
	const headers = Object.keys(PRODUCT_DATA[0]).filter(key => key !== 'id').map(key => key.charAt(0).toUpperCase() + key.slice(1));
							...
							>
								{headers.map(header => (
									<td key={header} className='px-6 py-4 whitespace-nowrap text-sm text-gray-300'>
										{header === 'Price' ? `£${product[header.toLowerCase()].toFixed(2)}` : product[header.toLowerCase()]}
									</td>
								))}
